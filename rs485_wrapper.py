...
class RS485:
    ...
    def write(self, data):
        ...
        # Short delay to ensure DE pin has time to switch
        time.sleep(0.005)  # 5ms delay for DE pin switching
        
        # Write data
        bytes_written = self.uart.write(data)
        
        # Wait for transmission to complete
        # CircuitPython buffers UART output, so we ensure it's sent
        time.sleep(0.010)  # 10ms for transmission
